...
public final class InvoicingData {
    ...
    public static InvoiceChange createInvoiceStatusChange(dev.vality.reporter.domain.enums.InvoiceStatus status) {
    ...
    private static InvoiceChange createPaymentStatusChange(String paymentId,
                                                           dev.vality.damsel.domain.InvoicePaymentStatus status) {
    ...
    private static InvoiceChange createRefundStatusChange(
            String paymentId,
            String refundId,
            dev.vality.damsel.domain.InvoicePaymentRefundStatus status
    ) {
        InvoicePaymentChange invoicePaymentChange = new InvoicePaymentChange();
        invoicePaymentChange.setId(paymentId);
    ...
    private static InvoiceChange createAdjustmentStatusChange(
            String paymentId,
            String adjustmentId,
            dev.vality.damsel.domain.InvoicePaymentAdjustmentStatus status
    ) {
        InvoicePaymentChange invoicePaymentChange = new InvoicePaymentChange();
        invoicePaymentChange.setId(paymentId);
    ...
    private static dev.vality.damsel.domain.Invoice createDomainInvoice(String invoiceId) {
        var hgInvoice = new dev.vality.damsel.domain.Invoice();
    ...
    private static dev.vality.damsel.payment_processing.InvoicePayment createInvoicePayment(
            String paymentId,
            String refundId,
            String adjustmentId
    ) {
        var invoicePayment = new dev.vality.damsel.payment_processing.InvoicePayment();
    ...
    private static dev.vality.damsel.domain.InvoicePayment createDamselInvoicePayment(String paymentId) {
        ...
        paymentResourcePayer.setResource(new DisposablePaymentResource()
                ...
                .setPaymentTool(PaymentTool.digital_wallet(new DigitalWallet()
                        .setId("123")
                        .setPaymentService(new PaymentServiceRef("stripe"))))
        );
    ...
    private static List<dev.vality.damsel.domain.InvoicePaymentAdjustment> createInvoicePaymentAdjustmentList(
            String defaultAdjustmentId
    ) {
        List<dev.vality.damsel.domain.InvoicePaymentAdjustment> adjList = new ArrayList<>();
    ...
    private static dev.vality.damsel.domain.InvoicePaymentAdjustment createInvoicePaymentAdjustment(
            String id,
            InvoicePaymentAdjustmentStatus status
    ) {
        var adj = new dev.vality.damsel.domain.InvoicePaymentAdjustment();
        adj.setId(id);
    ...
    private static List<dev.vality.damsel.payment_processing.InvoicePaymentRefund> createInvoicePaymentRefundList(
            String defaultRefundId
    ) {
        List<dev.vality.damsel.payment_processing.InvoicePaymentRefund> refundList = new ArrayList<>();
    ...
    private static dev.vality.damsel.payment_processing.InvoicePaymentRefund createInvoicePaymentRefund(
            String id,
            InvoicePaymentRefundStatus status
    ) {
        var hgRefund = new dev.vality.damsel.domain.InvoicePaymentRefund();
        hgRefund.setId(id);
        ...
        var refund = new dev.vality.damsel.payment_processing.InvoicePaymentRefund();
    ...
    private static List<dev.vality.damsel.domain.FinalCashFlowPosting> createCashFlowList() {
        return createNewCashFlow();
    }

    private static dev.vality.damsel.domain.PaymentRoute createRoute() {
        var paymentRoute = new dev.vality.damsel.domain.PaymentRoute();
    ...
    private static List<InvoicePaymentSession> createSessions() {
        ...
        invoicePaymentSessionList.add(createSession(
                dev.vality.damsel.domain.TargetInvoicePaymentStatus.processed(new InvoicePaymentProcessed()),
                false)
        );
        invoicePaymentSessionList.add(createSession(
                dev.vality.damsel.domain.TargetInvoicePaymentStatus.processed(new InvoicePaymentProcessed()),
        ...
        invoicePaymentSessionList.add(createSession(
                dev.vality.damsel.domain.TargetInvoicePaymentStatus.captured(new InvoicePaymentCaptured()),
                true)
        );
        return invoicePaymentSessionList;
    }

    private static InvoicePaymentSession createSession(dev.vality.damsel.domain.TargetInvoicePaymentStatus status,
    ...
    public static class InvoiceChangeStatusInfo {
        ...
        private int count;
        private dev.vality.reporter.domain.enums.InvoiceStatus status;
    ...
    public static class PaymentChangeStatusInfo {
        ...
        private dev.vality.damsel.domain.InvoicePaymentStatus status;
    ...
    public static class RefundChangeStatusInfo {
        ...
        private dev.vality.damsel.domain.InvoicePaymentRefundStatus status;
    ...
    public static class AdjustmentChangeStatusInfo {
        ...
        private dev.vality.damsel.domain.InvoicePaymentAdjustmentStatus status;
