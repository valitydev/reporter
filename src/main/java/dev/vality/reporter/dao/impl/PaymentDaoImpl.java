...
import static dev.vality.reporter.domain.tables.Payment.PAYMENT;
import static dev.vality.reporter.domain.tables.PaymentAdditionalInfo.PAYMENT_ADDITIONAL_INFO;
import static dev.vality.reporter.domain.tables.PaymentAggsByHour.PAYMENT_AGGS_BY_HOUR;
...
public class PaymentDaoImpl extends AbstractDao implements PaymentDao {
    ...
    private static PaymentFundsAmount mapFundsResultToPaymentFundsAmount(
        ...
        return Optional.ofNullable(fundsResult)
                .map(result -> new PaymentFundsAmount(
                        AccountingDaoUtils.getFunds(result.value1()), AccountingDaoUtils.getFunds(result.value2())))
